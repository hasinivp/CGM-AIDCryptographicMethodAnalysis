...
STPA Sec is a security analysis framework developed at MIT that allows analysts to look at a system through a top-down lens focusing on control-feedback loops instead of analyzing vulnerabilities component by component. Moreover, this framework allows the analyst to understand how each failure in communication systems/feedback loops can lead to hazards/losses. This type of analysis is often applied to medical devices, and an analysis with a focus on CGM/AID communication was completed in order to build an informed evaluation criteria for security protocols.

\subsection{Encryption Algorithms Evaluated[1][8]}\label{key}
...
This modern AEAD encryption is very similar to AES-CCM, and differs only in the use of the authentication format. AES-GCM makes use of the GHASH for authentication.
\subsubsection{ChaCha20Poly1305}
This method works similarly to AES-CCM/GCM, but runs on the most software-optimized algorithms.
\section{Methodology}
...
\item \texttt{results\_plot.py}: Used numpy and matplotlib to plot results of the simulation.
\end{itemize}
\vspace{.2cm}
Essentially, the simulation one symmetric encryption algorithm per session. At the beginning of each session the sensor and insulin pump are initialized and introduced to each other via an ECDH asymmetric key exchange. Each device then has a key that is used to encrypt/decrypt messages sent via symmetric encryption algorithms.
\subsection{Benchmarking Metrics}
\begin{itemize}
    \item Latency: the time/reponse delay between providing plain text and receiving encrypted ciphertext or from ciphertext to plaintext (both encryption and decryption latency was measured using time module in python)
    \item Memory: measured using python module memray
\end{itemize}
\subsection{STPA-Sec Analysis}
\vspace{.2cm}
\textbf{Identifying Losses}\\
In STPA losses are defined as "something of value to stakeholders" [4]. The primary stakeholders in this system are the patient, healthcare provider, and system manufacturers. The losses identified are below.
\begin{itemize}
...
\begin{itemize}
    \item SC-1: Integrity of messages must be preserved - preventing tampering of messages 
    \item SC-2: Authentication must be in place to prevent malicious actors to gain access to private health information
    \item SC-3: Input must be validated and limited to prevent flooding (denial of service type attacks)
    \item SC-4: Appropriate warnings to patient and precautionary measures must be set to avoid full depletion of computational resources (example: warning days/weeks in advance to patient to prevent full use of resources)
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.1\linewidth]{STPAControlStructure.pdf}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\linewidth}{|X|X|X|X|X|}
        ...
        \hline
        Transmission of Glucose Reading & Reading blocked from being transmitted & Tampered reading is accepted & Replayed reading is transmitted & Controller flooded with readings-DDoS \\
        \hline
        Sensor reading glucose levels & Sensor unable to identify glucose level & Incorrect measurement & - & Sensor unable to store new readings \\
        \hline
        Glucose data/trends to patient device & Patient not alerted & False alert issued to patient & - & -\\
        \hline
        Insulin delivery to patient & Insulin is not sent via actuator to patient & Over/under dose of insulin provided & - & - \\
        \hline
        Patient override to controller commands & - & Unauthorized override sent & Patient override is sent too late & -\\
        \hline
    \end{tabularx}
\end{table}
\textbf{Casual Scenarios \& Engineering Implication}\\
The focus of this specific study is the first row of unsafe control actions - derived from the CGM to AID glucose reading communication. \\

\begin{itemize}
    \item UCA: Reading blocked from being transmitted
    \item Scenario: No glucose reading is received by the AID controller, forcing it to blindly calculate insulin.
    \item Associated Casual Factors: Incorrect insulin delivered to patient, patient data loss.
\end{itemize}

Engineering Implications: The communication channel should be able to detect missing readings and alert patient immediately while transitioning to a safe/baseline mode. \\
\vspace{.1cm}

\begin{itemize}
    \item UCA: Tampered reading is accepted by AID controller
    \item Scenario: A modified glucose reading is processed and accepted 
    \item Associated Casual Factors: 
\end{itemize}

Engineering Implications: Integrity and authentication of messages must be maintained to avoid tampering of data.\\
\vspace{.1cm}

\begin{itemize}
    \item UCA: Replayed reading is transmitted to AID
    \item Scenario: A previous glucose reading is accepted as current, leading to incorrect insulin amount calculated.
    \item Associated Casual Factors: Timestamp not checked; nonce/counter validation missing.
\end{itemize}

Engineering Implications: The communication channel should verify timestamp, enable replay protection, and use counter/nonce verification to avoid such attacks.\\
\vspace{.1cm}
\begin{itemize}
    \item UCA: controller is flooded with readings 
    \item Scenario: The communication channel is flooded with readings, causing the AID algorithm to be overloaded with data.
    \item Associated Casual Factors: Lack of duplicate detection; AID algorithm failure.
\end{itemize}

Engineering Implications: Abnormal traffic should be detected and mitigated by the AID algorithm.\\

\subsection{Encryption Algorithm Choices}
The encryption algorithms chosen are a direct result of the engineering implications of the STPA-Sec analysis. To understand how each encryption algorithm works, refer back to \ref{key}. 
\begin{itemize}
    \item AES-CBC: chosen as purely a baseline legacy algorithm as this does not meet the requirements of the STPA-Sec analysis (Confidentiality-Only algorithm)
    \item AES-CBC + HMAC: Acts as a legacy authentication algorithm that was previously used for medical device security
    \item AES-CCM: Bluetooth Low Energy (BLE) standard - most commonly used in artificial pancreas system  
    \item AES-GCM: Modern security "gold-standard"
    \item ChaCha20Poly1305: Most software optimized encryption algorithms, often used by DIY hackers
\end{itemize}
...
\begin{figure}[H]
    ...
    \label{fig:placeholder}
\end{figure}
The memory benchmark shows that AES-GCM \& the ChaCha algorithm have the highest memory overhead, with AES-CCM using the lowest. This specific graph does not show significant differences between different key sizes, there will be a computational overhead in terms of storing the additional key/cipher text bits for the small embedded systems.

\section{Discussion}
\subsection{AES Accelerators}
One thing to note when discussing the implementation of these algorithms in an embedded system versus computer hardware is the presence of AES accelerators. Most modern day computer hardware (including the computer the simulation was run on) have AES accelerators, which significantly improves the time/computational overhead of AES based encryption systems. While many modern day artificial pancreas systems do have some form of an AES accelerator, in systems that do not have this compoenent, we may see that the ChaCha algorithm proves to be the superior choice.
\subsection{Limitations}
The largest limitation of this study is its inability to give results accurate to how encryption algorithms will work on an embedded systems device/actual artificial pancreas system. 

\section{Conclusion}
Overall, with both memory and latency, AES-CCM was the most optimized algorithm, that still met all security requirements. This is in alignment with current practices, as AES-CCM is the primary encryption mechanism used in BLE. Moreover, with regard to latency, all modern-day algorithms have similar optimization making security be the top priority when choosing algorithms. 
\vspace{.2cm}
\subsection{Future Work}
Following this simulation, the next obvious step would be to take this simulation and implement it in a embedded systems device to better simulate the CGM/AID system. Additionally, expanding this simulation to analyze different forms of key exchange/asymmetric encryption algorithms will provide insight into that portion of the encryption process. Further expanding the scope to include how potential attacks can influence encryption algorithm behaviors, or how BLE connection drop will affect the communication pipeline will add to 
